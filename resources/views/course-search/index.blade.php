            ...
            <div class="card p-4 mt-3 shadow">
                <form id="handleSearch">
                    ...
                    <div class="d-flex justify-content-center px-5">
                        ...
                        ?>
                        <div class="search mx-1">
                            <select class="form-select" id="university" name="university" style="width: 150px;">
                        ...
                        ?>
                        <div class="search mx-1">
                            <select class="form-select" id="course" name="course" style="width: 150px;">
                        ...
                        <?php
                        $degree = Illuminate\Support\Facades\DB::connection('mysql2')->select('SELECT DISTINCT courses.level as levels FROM universities JOIN uni_course ON universities.id = uni_course.uni_id JOIN courses ON uni_course.course_id = courses.id WHERE 1 AND universities.name != "any" AND courses.title != "any" AND courses.level!="" AND universities.status="ON";');
                        ?>
                        <div class="search mx-1">
                            <select class="form-select" id="level" name="level" style="width: 150px;">
                                <option value="">Select Degree</option>
                                @foreach ($degree as $degrees)
                                    <option value="{{ $degrees->levels }}">{{ $degrees->levels }}</option>
                                @endforeach
                            </select>
                        </div>
                        <?php
                        ...
                        ?>
                        <div class="search mx-1">
                            <select class="form-select" id="stream" name="stream" style="width: 150px;">
            ...
            <div class="card p-4 mt-3 shadow">
                ...
                <div>
                    <div class="search my-1">
                        ...
                        <input type="range" class="form-range" min="0" max="2000" step="250" value="0"
                            id="fees">
                    </div>
                    <div class="search my-1">
                        <label for="duration" class="form-label">Scholarships (0 to <span
                                id="scholarships_range"></span>)</label>
                        <input type="range" class="form-range" min="0" max="2000" step="100" value="0"
                            id="duration">
                    </div>
                </div>
            </div>
...
@section('script')
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#country,#degree,#stream,#intake,#university,#courses').on('change', function(e) {
                e.preventDefault();
                search();
            });
            ...
            $('#university').change(function() {
                ...
                if (university != "") {
                    ...
                    $.ajax({
                        url: "https://bizzeducation.com/registration/intake-month",
                        method: "GET",
                        data: {
                            university: university
                        },
        ...
        function events(page) {
            ...
            let url = '';
            let search = '';
            if (country != "" || degree != "" || stream != "" || intake != "" || university != "" || courses != "") {
                if (country != "") {
                    if (search == "") {
                ...
                url = 'courses/universities-api-search?' + search + '&page=' + page
            } else {
                url = 'courses/universities-api-search?page=' + page
            }
            console.log(url);
            $.ajax({
                url: url,
        ...
        function search() {
            ...
            //console.log('country: '+country+'degree: '+degree+'stream: '+stream+'intake: '+intake+'unversity: '+unversity+'courses: '+courses);
